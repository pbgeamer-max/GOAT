...
const RustItemSlot: React.FC<{ item?: KitItem; compact?: boolean }> = ({ item, compact = false }) => {
  const [imgSrc, setImgSrc] = useState<string>(() => {
    ...
    return `https://rustlabs.com/img/items180/${item.Shortname.toLowerCase().trim()}.png`;
  });
  ...
  useEffect(() => {
    if (item?.Shortname) {
      setImgSrc(`https://rustlabs.com/img/items180/${item.Shortname.toLowerCase().trim()}.png`);
      setHasError(false);
    }
  }, [item?.Shortname]);
    ...
    >
      {!hasError ? (
        <img
          ...
          loading="lazy"
          onError={() => {
            if (imgSrc.includes("rustlabs.com")) {
              setImgSrc(`https://files.rustyhub.de/rust/icons/${cleanShortname}.png`);
            } else if (imgSrc.includes("rustyhub.de")) {
              setImgSrc(`https://cdn.jsdelivr.net/gh/Facepunch/Rust-Item-Icons/items/${cleanShortname}.png`);
            } else {
              setHasError(true);
            }
          }}
                  ...
                  {(inspectKit.Items || []).map((it, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-2 truncate">
                        <img
                          src={`https://rustlabs.com/img/items180/${it.Shortname.toLowerCase().trim()}.png`}
                          alt=""
